...
function Post_GetPostById(post_id, res){
    postDao.Post_GetPostById(post_id, connection, function(response){
        if(response == 701)
            res.json(new ResponseData(701, "Error!", ""));
        else
            var listPost = [];
...
function Post_Insert(post, res){
    postDao.Post_Insert(post, connection, function(response){
        if(response == 701)
            ...
            res.json(new ResponseData(200, "Success", response.post_id));
    });
}
